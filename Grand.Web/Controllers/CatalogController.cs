...
namespace Grand.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public virtual async Task<IActionResult> Category(string categoryId, CatalogPagingFilteringModel command)
        {
            ...
            //template
            var templateViewPath = await _mediator.Send(new GetCategoryTemplateViewPath()
                {TemplateId = category.CategoryTemplateId});
        ...
        public virtual async Task<IActionResult> Manufacturer(string manufacturerId,
            CatalogPagingFilteringModel command)
        {
            ...
            if (!manufacturer.Published &&
                !await _permissionService.Authorize(StandardPermissionProvider.ManageManufacturers, customer))
                return InvokeHttp404();
            ...
            //activity log
            await _customerActivityService.InsertActivity("PublicStore.ViewManufacturer", manufacturer.Id,
                _localizationService.GetResource("ActivityLog.PublicStore.ViewManufacturer"), manufacturer.Name);
            await _customerActionEventService.Viewed(customer, HttpContext.Request.Path.ToString(),
                Request.Headers[HeaderNames.Referer].ToString() != null
                    ? Request.Headers[HeaderNames.Referer].ToString()
                    : "");
            ...
            //template
            var templateViewPath = await _mediator.Send(new GetManufacturerTemplateViewPath()
                {TemplateId = manufacturer.ManufacturerTemplateId});
        ...
        public virtual async Task<IActionResult> VendorReviews(string vendorId)
        {
            ...
            if (_workContext.CurrentCustomer.IsGuest() && !_vendorSettings.AllowAnonymousUsersToReviewVendor)
                ModelState.AddModelError("",
                    _localizationService.GetResource("VendorReviews.OnlyRegisteredUsersCanWriteReviews"));
            //default value
        ...
        public virtual async Task<IActionResult> Filter([FromForm(Name = "categoryId")] string categoryId,
            ...
            [FromForm(Name = "minPrice")] string minPrice, [FromForm(Name = "attributes")] string[] attributes)
        {
            ...
            foreach (var i in attributes)
            {
                var key = i.Split(':')[0];
                var value = i.Split(':')[1];
                if (attributesList.Keys.Contains(key))
                    attributesList[key].Add(value);
                else
                    attributesList.Add(key, new List<string>() {value});
            }

            string str = attributesList.Keys.Aggregate("",
                (current, key) => current + $"{key} : {string.Join(',', attributesList[key])}");
            var category = await _categoryService.GetCategoryById(categoryId);
            if (category == null)
                return InvokeHttp404();
            ...
            model.Products = model.Products.Select(item => item)
                .Where(item => item.ProductPrice.PriceValue > decimal.Parse(minPrice))
                .Where(item => item.ProductPrice.PriceValue < decimal.Parse(maxPrice)).ToList();
            if (attributes.Length <= 0) return View(templateViewPath, model);
            bool remove = true;
            foreach (var prod in model.Products.ToList())
            {
                remove = true;
                foreach (var attr in prod.ProductAttributeModels)
                {
                    if (attributesList.ContainsKey(attr.Name))
                    {
                        foreach (var attrValue in attr.Values)
                        {
                            if (attributesList[attr.Name].Contains(attrValue.Name))
                            {
                                remove = false;
                                break;
                            }
            ...
            return Content(string.Join(" --- ",
                model.Products.Select(item => string.Join(',', item.ProductAttributeModels.Select(item => item.Name)))));
        return View(templateViewPath, model);   
        }
        ...
        [ValidateCaptcha]
        public virtual async Task<IActionResult> VendorReviewsAdd(string vendorId, VendorReviewsModel model,
            bool captchaValid,
            [FromServices] CaptchaSettings captchaSettings)
        {
            ...
            if (_vendorSettings.VendorReviewPossibleOnlyAfterPurchasing &&
                ...
                ModelState.AddModelError(string.Empty,
                    _localizationService.GetResource("VendorReviews.VendorReviewPossibleOnlyAfterPurchasing"));
            ...
            if (ModelState.IsValid)
            {
                ...
                await _customerActivityService.InsertActivity("PublicStore.AddVendorReview", vendor.Id,
                    _localizationService.GetResource("ActivityLog.PublicStore.AddVendorReview"), vendor.Name);
        ...
        public virtual async Task<IActionResult> SetVendorReviewHelpfulness(string VendorReviewId, string vendorId,
            bool washelpful, [FromServices] ICustomerService customerService)
        {
            var vendor = await _vendorService.GetVendorById(vendorId);
